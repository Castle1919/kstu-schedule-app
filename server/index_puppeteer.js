...
async function scrapeSchedule(username, password) {
    ...
    try {
        await page.waitForSelector('a[href="/lang/change/ru/"]', { timeout: 5000 });
    } catch (e) {
        // If we can't find the language switcher within 5s, we probably aren't logged in correctly or are on the wrong page
        console.log('Language switcher not found, assuming login failed or page changed structure.');
        await browser.close();
        throw new Error('INVALID_CREDENTIALS');
    }
